...
def getInput():
      ...
      runQuery(start_date, start_time, end_date, end_time, next, limit)

def runQuery(start_date, start_time, end_date, end_time, next, limit): 
   # 
   s = ICDMConnector()
   url = "/v1/event-search"
   m = "POST"
   data = "{ \"start_date\": \"" + start_date + "T" + start_time + ".000+0000\",  \"end_date\": \"" + end_date + "T" + end_time + ".000+0000\", \"next\": " + str(next) + ", \"limit\": " + str(limit) + ", \"product\":\"SAEP\", \"feature_name\":\"ALL\" }"
...
getInput()
# else:
# runQuery(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
